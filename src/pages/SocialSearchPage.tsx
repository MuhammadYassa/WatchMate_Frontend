...
import { Search as SearchIcon, UserRoundSearch, Users } from 'lucide-react'
        ...
        <div className="relative space-y-5">
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-base leading-7 text-[color:var(--color-text-secondary)]">
              Search by username to open a profile and keep your WatchMate circle close to the titles you care about.
            </p>
          </div>
      ...
      {debouncedQuery && searchQuery.isError ? (
        <div className="relative z-10">
          <ErrorState
            action={
              <Link
                className="inline-flex min-h-11 items-center rounded-2xl border border-white/10 px-5 text-sm font-semibold text-[color:var(--color-text-primary)] transition hover:border-white/18 hover:bg-[rgba(255,255,255,0.08)] hover:text-white"
                to="/profile"
              >
                Back to profile
              </Link>
        ...
        <section className="relative z-10 space-y-5">
          ...
          <div className="grid gap-4">
            {results.map((result) => (
              ...
              >
                <Card className="flex flex-col gap-5 border-white/10 bg-[linear-gradient(145deg,rgba(20,21,25,0.92)_0%,rgba(12,13,17,0.96)_100%)] p-5 transition duration-200 ease-out hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(0,0,0,0.38)] md:flex-row md:items-center md:justify-between">
                  ...
                  <span className="text-sm font-medium text-[color:var(--color-accent)]">Open profile</span>
                </Card>
              </Link>
            ))}
          </div>
