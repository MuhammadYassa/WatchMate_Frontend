...
function WatchlistsLoadingState() {
  return (
    <PageContainer className="relative isolate space-y-8 overflow-hidden pt-8 md:pt-12">
      ...
      {[0, 1, 2].map((item) => (
        <Card
          className="relative z-10 space-y-6 border-white/10 bg-[linear-gradient(145deg,rgba(20,21,25,0.92)_0%,rgba(12,13,17,0.96)_100%)] p-6"
          key={item}
        >
...
function WatchlistPosterPreview({ watchlist }: { watchlist: WatchListDTO }) {
  ...
  if (!previewItems.length) {
    return (
      <div className="grid min-h-[220px] place-items-center rounded-[28px] border border-dashed border-white/10 bg-[rgba(255,255,255,0.03)] p-5">
  ...
  return (
    <div className="relative flex min-h-[220px] items-center justify-center overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,rgba(20,21,25,0.94)_0%,rgba(11,12,16,0.98)_100%)] px-6 py-5">
      {previewItems.map((item, index) => {
        const rotation = index === 0 ? '-rotate-6 -translate-x-8' : index === 2 ? 'rotate-6 translate-x-8' : 'z-10 -translate-y-2'

        return hasImagePath(item.posterPath) ? (
          <img
            key={`${item.type}-${item.tmdbId}`}
            alt=""
            className={`absolute h-[170px] w-[112px] rounded-[20px] border border-white/10 object-cover shadow-[0_22px_50px_rgba(0,0,0,0.34)] transition duration-300 ${rotation}`}
            src={getPosterUrl(item.posterPath, 'w342') ?? undefined}
          />
        ) : (
          <PosterPlaceholder
            key={`${item.type}-${item.tmdbId}`}
            className={`absolute h-[170px] w-[112px] ${rotation}`}
            title={item.title}
          />
        )
      })}
...
function WatchlistEditorDialog({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto bg-[rgba(5,6,8,0.82)] px-4 py-4 backdrop-blur-md sm:items-center sm:py-8">
      <Card className="w-full max-w-xl space-y-5 border-white/10 bg-[linear-gradient(145deg,rgba(18,19,23,0.94)_0%,rgba(10,11,15,0.98)_100%)] p-5 shadow-[0_40px_100px_rgba(0,0,0,0.45)] sm:p-6">
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <Button className="w-full sm:w-auto" disabled={pending || value.trim().length < 2} onClick={() => onSubmit(value.trim())}>
            {submitLabel}
          </Button>
        </div>
      </Card>
  ...
  return (
    <PageContainer className="relative isolate space-y-8 overflow-hidden pt-8 md:space-y-10 md:pt-12">
        ...
        <div className="relative z-10 flex flex-col gap-6 p-6 md:flex-row md:items-end md:justify-between md:p-8">
          <div className="max-w-2xl space-y-4">
            ...
            <p className="text-base leading-7 text-[color:var(--color-text-secondary)]">
              Build collections around moods, nights in, rewatches, and whatever you want queued up next.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              className="inline-flex min-h-11 items-center rounded-2xl border border-white/10 bg-[rgba(255,255,255,0.04)] px-5 text-sm font-semibold text-[color:var(--color-text-primary)] transition hover:border-white/18 hover:bg-[rgba(255,255,255,0.08)] hover:text-white"
              to="/favourites"
            >
      ...
      {watchlists.length === 0 ? (
        <div className="relative z-10">
          <EmptyState
            action={
              <div className="flex flex-wrap gap-3">
                <Link className="inline-flex min-h-11 items-center rounded-2xl bg-[color:var(--color-accent)] px-5 text-sm font-semibold text-[#122f5f] transition hover:bg-[color:var(--color-accent-strong)]" to="/search">
                  Search titles
                </Link>
                <Link className="inline-flex min-h-11 items-center rounded-2xl border border-white/10 px-5 text-sm font-semibold text-[color:var(--color-text-primary)] transition hover:border-white/18 hover:bg-[rgba(255,255,255,0.08)] hover:text-white" to="/discover">
                  Explore picks
                </Link>
              </div>
            }
        ...
        <div className="relative z-10 grid gap-6">
          {watchlists.map((watchlist) => {
            ...
            return (
              <Card
                className="space-y-6 border-white/10 bg-[linear-gradient(145deg,rgba(20,21,25,0.92)_0%,rgba(12,13,17,0.96)_100%)] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.32)] md:p-6"
                key={watchlist.id}
              >
                <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
                  ...
                  <div className="space-y-5 self-center">
                    ...
                    <p className="max-w-2xl text-sm leading-7 text-[color:var(--color-text-secondary)]">
                      Open the list to browse every saved title and remove anything you no longer want in rotation.
                    </p>
                  </div>
                </div>
                ...
                {expanded ? (
                  watchlist.media.length > 0 ? (
                    <div className="space-y-5 rounded-[28px] border border-white/10 bg-[rgba(255,255,255,0.03)] p-5">
                      <SectionHeader eyebrow="Saved titles" title="Inside this collection" />
                      <MediaGrid className="gap-5">
                        {watchlist.media.map((item) => (
                          <Card
                            className="space-y-4 border-white/10 bg-[rgba(255,255,255,0.03)] p-4"
      ...
      {watchlistsPage.totalPages > 1 ? (
        <Card className="relative z-10 flex flex-wrap items-center justify-between gap-3 border-white/10 bg-[rgba(255,255,255,0.03)] px-5 py-4">
