...
export function SeasonCard({ season, showTmdbId }: SeasonCardProps) {
  ...
  return (
    <Link className="group block focus-visible:outline-none" to={`/shows/${showTmdbId}/seasons/${season.seasonNumber}/episodes`}>
      <Card className="grid gap-5 border-white/10 bg-[linear-gradient(145deg,rgba(20,21,25,0.92)_0%,rgba(12,13,17,0.96)_100%)] p-4 transition duration-300 group-hover:-translate-y-1 group-hover:border-white/16 group-hover:shadow-[0_30px_70px_rgba(0,0,0,0.42)] sm:grid-cols-[148px_1fr] sm:p-5">
        <div className="relative max-w-[148px] overflow-hidden rounded-[24px] border border-white/10 bg-[rgba(255,255,255,0.03)] shadow-[0_18px_35px_rgba(0,0,0,0.28)]">
        ...
        <div className="space-y-4 self-center">
          ...
          <div className="flex flex-wrap gap-3 text-sm text-[color:var(--color-text-secondary)]">
            ...
            {season.airDate ? (
              <span className="inline-flex items-center rounded-[14px] border border-white/10 bg-[rgba(255,255,255,0.04)] px-3 py-2">
